...
error_t parser_del(int key, char *arg, struct argp_state *state)
{
        ...
        switch (key) {
                case (OPT_DEL_SOFT_DELETE):
                        ...
                        return 0;
                case (ARGP_KEY_ARG):
                        if (del_arguments->path != 0
                            && del_arguments->path[0] != '\0')
                                argp_usage(state);
...
error_t parser_get(int key, char *arg, struct argp_state *state)
{
        ...
        switch (key) {
                ...
                case (ARGP_KEY_ARG):
                        ...
                        if (get_arguments->dest == 0 ||
                            get_arguments->dest[0] == '\0') {
...
error_t parser_list(int key, char *arg, struct argp_state *state)
{
        ...
        switch (key) {
                case (ARGP_KEY_ARG):
                        if (list_arguments->path != 0 &&
                            list_arguments->path[0] != '\0')
                                argp_usage(state);
                ...
                case ARGP_KEY_END:
                        switch (arguments->command) {
                                case COMMAND_NONE:
                                        ...
                                        break;
                                case COMMAND_DEL:
                                        if (arguments->del_arguments->path ==
                                            0) {
                                                argp_usage(state);
                                        }
                                        break;
                                case COMMAND_GET:
                                        ...
                                        break;
                                case COMMAND_MOVE:
                                        if (arguments->move_arguments->source ==
                                            0 ||
